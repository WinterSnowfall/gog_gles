        ...
        if response.status_code == ConstantsInterface.HTTP_OK:
            ...
            if entry_count == 0:
                ...
                # manual scans will be treated as update scans
                    ...
                    # we should never find ourselves in a situation where we have no v2 data returned for
                    # an id that already has a v2 json entry stored in the database, however if that
                    # ever happens, do not overwrite the entry and simply skip processing
                    if v2_query_response is None and existing_v2_json_formatted is not None:
                        logger.error(f'{process_tag}PQ >>> Missing v2 payload for {product_id}: {product_title}.')

                    elif existing_json_formatted != json_formatted:
