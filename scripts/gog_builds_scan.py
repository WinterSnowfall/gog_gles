...
def gog_builds_query(process_tag, product_id, os_value, scan_mode,
    ...
    try:
        ...
        if https_proxy:
            response = session.get(builds_url, headers=ConstantsInterface.HEADERS, cookies=ConstantsInterface.COOKIES,
            ...
            if total_count > 0:
                ...
                if not (entry_count == 1 and (scan_mode == 'full' or scan_mode == 'products')):
                    ...
                    if len(json_parsed['items']) != 0:
                        ...
                        main_version_names = ConstantsInterface.MVF_VALUE_SEPARATOR.join(main_item_list)
                        branch_version_names = ConstantsInterface.MVF_VALUE_SEPARATOR.join(branch_item_list)
                ...
                if entry_count == 0:
                    # gb_int_nr, gb_int_added, gb_int_removed, gb_int_updated,
                    # gb_int_json_payload, gb_int_json_diff, gb_int_id, gb_int_title,
                    # gb_int_os, gb_total_count, gb_count, gb_main_version_names,
                    # gb_branch_version_names, gb_has_private_branches
                    with db_lock:
                        db_cursor.execute(INSERT_BUILD_QUERY, (None, datetime.now().isoformat(' '), None, None,
                                                               json_formatted, None, product_id, product_name,
                                                               os_value, total_count, count, main_version_names,
                    ...
                    # manual scans will be treated as update scans
                        ...
                        if existing_json_formatted != json_formatted:
                            ...
                            with db_lock:
                                db_cursor.execute(UPDATE_BUILD_QUERY, (datetime.now().isoformat(' '), json_formatted, diff_formatted,
                    ...
                    if existing_delisted is None:
                        ...
                        with db_lock:
                            ...
                            db_cursor.execute('UPDATE gog_builds SET gb_int_removed = ?, gb_int_json_diff = NULL, gb_total_count = 0, gb_count = 0, '
                                              ...
                                              'WHERE gb_int_id = ? AND gb_int_os = ?', (datetime.now().isoformat(' '), product_id, os_value))
                            db_connection.commit()
    ...
    # atempt a proxy restart in case of a timeout by raising an exception to the workload_dispatcher
        ...
        logger.warning(f'{process_tag}BQ >>> HTTP request timed out after {HTTP_TIMEOUT} seconds for {product_id}, {os_value}.')
        raise Exception()
    ...
    # sometimes the HTTPS connection gets rejected/terminated
        ...
        # HTTP 429/509 errors will be returned in case of throttling
            ...
            while not terminate_event.is_set():
                ...
                while not retries_complete and not terminate_event.is_set():
                    ...
                    if not terminate_event.is_set():
                        try:
                            retries_complete = gog_builds_query(process_tag, product_id, FULL_SCAN_OS_VALUE, scan_mode, https_proxy,
                                                                db_lock, processSession, process_db_connection)
                        # exceptions should only be raised to trigger a proxy restart
                        except:
                            if https_proxy:
        ...
        try:
            with sqlite3.connect(DB_FILE_PATH) as db_connection:
                ...
                for delta_entry in delta_list:
                    ...
                    current_main_version_names = delta_entry[3].split(ConstantsInterface.MVF_VALUE_SEPARATOR)
                        ...
                        if not excluded and current_latest_file_version != current_latest_build_version:
                            ...
                            if installer_delta_entry_count != 0:
                                ...
                                if installer_version_delta_entry_count != 0:
                                    ...
                                    db_cursor.execute(UPDATE_INSTALLERS_DELTA_QUERY, (datetime.now().isoformat(' '), current_latest_build_version_orig,
                                ...
                                db_cursor.execute(INSERT_INSTALLERS_DELTA_QUERY, (None, datetime.now().isoformat(' '), None, None, current_product_id, current_product_title,
                ...
                for discrepancy in discrepancy_list:
                    ...
                    if current_product_id not in detected_discrepancies[current_os_value]:
                        ...
                        db_cursor.execute('UPDATE gog_installers_delta SET gid_int_fixed = ?, gid_int_false_positive = 0, gid_int_false_positive_reason = NULL '
                                          'WHERE gid_int_id = ? AND gid_int_os = ? AND gid_int_fixed IS NULL',
                                          (datetime.now().isoformat(' '), current_product_id, current_os_value))
                        db_connection.commit()
